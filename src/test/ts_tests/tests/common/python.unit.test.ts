...
suite('PythonEnvironments.api() Integration Tests', () => {
    let pythonEnvsApiStub: sinon.SinonStub;
    let pythonExtensionApiStub: sinon.SinonStub;

    setup(() => {
        resetCachedApis();
        pythonEnvsApiStub = sinon.stub(PythonEnvironments, 'api');
        pythonExtensionApiStub = sinon.stub(PythonExtension, 'api');
    });
    ...
    test('Uses PythonEnvironments.api() to resolve interpreter', async () => {
        const mockEnvsApi = {
            getEnvironment: sinon.stub().resolves({
                version: '3.12.0',
                execInfo: {
        ...
        pythonEnvsApiStub.resolves(mockEnvsApi);
        ...
        assert.strictEqual(result.path![0], '/usr/bin/python3.12');
        assert.isTrue(mockEnvsApi.getEnvironment.calledOnce);
        assert.isTrue(pythonExtensionApiStub.notCalled, 'Legacy PythonExtension.api should not be called');
    });

    test('Falls back to legacy API when PythonEnvironments.api() throws', async () => {
        pythonEnvsApiStub.rejects(new Error('Extension not available'));
